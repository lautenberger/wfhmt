...
!          WRITE(*,*) '# of values to be sorted is not positive.'
...
!     +      1)
         WRITE(*,*) 'Sort control parameter is not 2, 1, -1, or -2'
         RETURN
      ENDIF
C
...
C
  ...
  200    CONTINUE
      ...
      SUBROUTINE DSORT_DOUBLE_PRECISION (DX, DY, N, KFLAG)
C***BEGIN PROLOGUE  DSORT
C***PURPOSE  Sort an array and optionally make the same interchanges in
C            an auxiliary array.  The array may be sorted in increasing
C            or decreasing order.  A slightly modified QUICKSORT
...
C
  ...
  200    CONTINUE
      ENDIF
      RETURN
      END SUBROUTINE DSORT_DOUBLE_PRECISION
